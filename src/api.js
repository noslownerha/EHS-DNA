...
export const api = {
  ...
  pointsFor: (userId) => req(`/points/${userId}`),
  redeemReward: (userId, points, description) => req("/recognition/redeem", { method: "POST", body: { userId, points, description } }),
  listRedemptions: (userId) => req(`/recognition/redemptions${userId ? `?userId=${userId}` : ""}`),
  ...
  opAttention: () => req("/op/attention"),
  opBillingOverview: () => req("/op/billing/overview"),
  ...
  bulkCreateDepartments: (rows) => req("/departments/bulk", { method: "POST", body: { rows } }),
  bulkCreateTrainingGroups: (rows) => req("/training-groups/bulk", { method: "POST", body: { rows } }),
